...
export default class Concern extends Component {
	...
	listItemRender () {
		...
		return (
			concernData.map(function(data,index){
				return (
					<Brand key = {index} img = {data.brandlogo} name = {data.name} detail = {data.brief} />
				)
			})
		)
	}
		...
		return (
			<div style = { styles.brands }>
				...
				<div style = { styles.goodsInfo }>
					...
					<p style = { styles.detail }>自动换行方法有待解决</p>
				</div>
				<span style = { styles.concern } onClick = {() => this.action()}>{state}</span>
			</div>
		);
	}
}
